    ...
    <main style={{color: "black", marginBottom: "1rem" }} className="min-h-screen bg-white p-8 max-w-2xl mx-auto relative">
      ...
      <form onSubmit={handleSubmit} className="space-y-10 mt-6">
        ...
        <section>
          ...
          <input type="file" accept="image/*"   title="Upload your headshot"
onChange={handleImageUpload} className="block mb-2" />
          {preview && (
...
function Select({ label, name, value, onChange, options }: any) {
  return (
    <div>
      <label className="block text-sm font-medium">{label}</label>
      <select name={name} value={value}   title="Select "
 onChange={onChange} className="w-full mt-1 p-2 border border-gray-300 rounded">
...
function Checkbox({ label, name, checked, onChange }: any) {
  return (
    <div className="flex items-center gap-2">
      <input type="checkbox"   title="Chechbox"
